  ...
  useEffect(() => {
    ...
    const fetchRepos = async () => {
      ...
      const response = await fetch(
        "https://api.github.com/users/neil-stuart/repos?type=owner&per_page=100"
      );
      const data = await response.json();
      // Extract name, html_url, description, and updated_at from each repo.
      var new_repos = data.map(
        ({ name, html_url, description, updated_at, language }) => ({
          name,
          html_url,
          description,
          updated_at,
          ...
          {!postData || !("meta" in postData) || postData.meta.length == 0 ? (
            ...
            postData.meta.map((project) => (
              <motion.div
                ...
                transition={{ ease: "easeOut", duration: 0.15 }}
              >
                <Post slug={project.slug} title={project.title} />
              </motion.div>
            ))
          )}
        </div>
